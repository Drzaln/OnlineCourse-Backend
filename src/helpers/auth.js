  ...
  accessToken: (req, res, next) => {
    ...
    jwt.verify(accessToken, secretKey, (err, decoded) => {
      if (err && err.name === 'TokenExpiredError') {
        return respon.response(res, null, 402, 'Token expired')
      }

      if (err && err.name === 'JsonWebTokenError') {
        return respon.response(res, null, 401, 'Invalid Token')
      }

      if (parseInt(userToken) !== parseInt(decoded.idUser)) {
        return respon.response(res, null, 401, 'Invalid User Token')
      }
      console.log(decoded)
      next()
    })
